@@ -2,86 +2,186 @@ const http = require('http');
const fs = require('fs');
...
async function updateItem(id, text) {
    try {
        await pool.query('UPDATE items SET text = ? WHERE id = ?', [text, id]);
    } catch (error) {
...
async function generateListRows() {
    ...
    return items.map((item, index) => `
        <tr id="row-${item.id}">
            ...
            <td>
                <button class="action-btn edit-btn" onclick="enableEdit(${item.id}, '${item.text.replace(/'/g, "\\'")}')">Edit</button>
